...
export default function GalleryTab({ items, onItemsChange }: TabComponentProps<GalleryItem>) {
  ...
  const validate = (form: Omit<GalleryItem, 'id'>): string | null => {
    if (!form.title) return t.galleryValidateTitle;
    if (!form.description) return t.galleryValidateDesc;
    if (!form.url && !form.localImage) return t.galleryValidateImg;
    return null;
  };
  ...
  const {
    ...
    { itemName: t.tabGallery, emptyForm: EMPTY_FORM, prefix: 'g_', validate },
    items, onItemsChange
  );
    ...
    <div className="space-y-8">
      <div className="bg-white border border-gray-100 rounded-3xl p-6 sm:p-8 shadow-sm">
        <h3 className="text-xl font-display font-extrabold text-marine-950 mb-6 flex items-center space-x-2">
          ...
          <span>{editingId ? t.galleryEdit : t.galleryAdd}</span>
        </h3>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="md:col-span-2">
            <ImageUploader
              ...
              label={t.imagen}
              uploadLabel={t.subirImagen}
              initialPreview={form.localImage || form.url || undefined}
            />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t.galleryTitleLabel}</label>
            <input type="text" placeholder={lang === 'es' ? 'ej. Coral Negro en María la Gorda' : 'e.g. Black Coral at María la Gorda'} value={form.title}
          ...
          <div className="md:col-span-2">
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t.galleryDescLabel}</label>
            <textarea rows={3} placeholder={lang === 'es' ? 'Describa el espécimen, el lugar o la historia...' : 'Describe the specimen, location, or story...'}
              value={form.description}
              onChange={(e) => setForm(prev => ({ ...prev, description: e.target.value }))}
              className="w-full border border-gray-200 px-4 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500" required />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">{t.galleryDateLabel}</label>
          ...
          <div>
            ...
            <input type="text" value={form.categoryEn || ''}
          ...
          <div className="md:col-span-2 flex justify-end space-x-3 pt-4 border-t border-gray-100">
            ...
            <button type="submit" disabled={isSubmitting}
              ...
              {editingId ? t.galleryUpdate : t.gallerySave}
            </button>
          </div>
        </form>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm">
        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest bg-gray-50 px-6 py-4 border-b border-gray-100">{t.galleryList}</h3>
