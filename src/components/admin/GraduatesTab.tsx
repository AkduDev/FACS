...
export default function GraduatesTab({ items, onItemsChange }: TabComponentProps<Graduate>) {
  ...
  const [form, setForm] = useState<Omit<Graduate, 'id'>>({
    name: '',
    certificationCode: '',
  ...
  const generateId = () => 'g_' + Math.random().toString(36).substring(2, 9);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      const newItem: Graduate = { id: generateId(), ...form };
      onItemsChange(prev => [newItem, ...prev]);
    }
    resetForm();
  };

  const startEdit = (item: Graduate) => {
      ...
      <div className="bg-white border border-gray-100 rounded-3xl p-6 sm:p-8 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select
              value={form.level}
              onChange={(e) => setForm({ ...form, level: e.target.value as Graduate['level'] })}
              className="w-full border border-gray-200 px-4 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500"
            >
