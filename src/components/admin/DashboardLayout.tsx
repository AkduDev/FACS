...
export default function DashboardLayout({ activeTab, onTabChange, onLogout, children }: DashboardLayoutProps) {
  ...
  const tabs: { id: AdminTab; label: string; icon: React.ReactNode }[] = [
    { id: 'gallery', label: t.tabGallery, icon: <Image className="h-4 w-4" /> },
    { id: 'news', label: t.tabNews, icon: <FileText className="h-4 w-4" /> },
    { id: 'events', label: t.tabEvents, icon: <Calendar className="h-4 w-4" /> },
    { id: 'instructors', label: t.tabInstructors, icon: <Users className="h-4 w-4" /> },
    { id: 'graduates', label: t.tabGraduates, icon: <GraduationCap className="h-4 w-4" /> },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-gradient-to-r from-marine-950 to-marine-900 p-6 sm:p-8 rounded-3xl text-white shadow-xl">
          <div>
            ...
            <h1 className="text-3xl font-display font-extrabold text-white mt-2 tracking-tight">{t.adminTitle}</h1>
            <p className="text-sm text-marine-200 mt-1">{t.adminSub}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          <div className="lg:col-span-3 bg-white border border-gray-100 rounded-3xl p-4 shadow-sm flex flex-col sm:grid sm:grid-cols-2 lg:flex lg:flex-col space-y-1.5 sm:space-y-0 sm:gap-2 lg:space-y-1.5">
            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest px-4 py-2 sm:col-span-2 lg:col-span-1">{t.modulosDatos}</h2>
            {tabs.map((tab) => (
              <button
                key={tab.id}
